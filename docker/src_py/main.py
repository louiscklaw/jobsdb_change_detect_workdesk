...
DS_AUTH_TOKEN = os.getenv("DS_AUTH_TOKEN")
...
try:
    ...
    SendJobAdDetail(api, chat_id, job_ad_details_md)
    NewDivider()

    # ASK_DEEPSEEK_FOR_COMPANY_BACKGROUND_INFORMATION
    AskDeepseekForCompanyBackgroundInformation(
        api, chat_id, result.id, fetched_company_name
    )
    NewDivider()

    TellDeepseekForCandiateBackground(api, chat_id)
    NewDivider()

    ConsolateInformationsAltogether(api, chat_id, result.id)
    NewDivider()

    answer_letter_md = DraftApplicationLetter(api, chat_id, result.id)
    UpdateDraftApplicationLetter(result.id, answer_letter_md)
